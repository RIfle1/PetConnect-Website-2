...
CREATE TABLE IF NOT EXISTS `APP DB`.`Address` (
  ...
  `adrPostalCode` CHAR(5) NOT NULL,
  `adrStreet` VARCHAR(80) NOT NULL,
  `adrBuilding` VARCHAR(4) NOT NULL,
  `adrApartmentNumber` VARCHAR(4) NOT NULL,
  `adrFloor` VARCHAR(2) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `APP DB`.`Device` (
  ...
  `Client_cltID` CHAR(255) NOT NULL,
  `Client_cltID1` CHAR(255) NOT NULL,
  PRIMARY KEY (`devID`, `Client_cltID`, `Client_cltID1`),
  INDEX `fk_Device_Client1_idx` (`Client_cltID1` ASC),
  CONSTRAINT `fk_Device_Client1`
    FOREIGN KEY (`Client_cltID1`)
    REFERENCES `APP DB`.`Client` (`cltID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `APP DB`.`Data_Device` (
  ...
  `Device_Client_cltID` CHAR(255) NOT NULL,
  `Device_Client_cltID1` CHAR(255) NOT NULL,
  PRIMARY KEY (`dapID`, `Device_devID`, `Device_Client_cltID`, `Device_Client_cltID1`),
  INDEX `fk_Data_Device_Device1_idx` (`Device_devID` ASC, `Device_Client_cltID` ASC, `Device_Client_cltID1` ASC),
  CONSTRAINT `fk_Data_Device_Device1`
    FOREIGN KEY (`Device_devID` , `Device_Client_cltID` , `Device_Client_cltID1`)
    REFERENCES `APP DB`.`Device` (`devID` , `Client_cltID` , `Client_cltID1`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `APP DB`.`Assistance` (
  ...
  `astAnswer` VARCHAR(255) NOT NULL,
  `astDate` DATE NOT NULL,
  PRIMARY KEY (`astID`))
ENGINE = InnoDB;
